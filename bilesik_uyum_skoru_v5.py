...
Cikti: data/backtest/bilesik_uyum_skoru_v5_sonuc.json
"""
import json
import datetime
...
# 25.09 TAM 30-HISSE ESLEME (kullanici arastirmasi + Claude dogrulamasi):
# kullanicinin sagladigi "en uygun ALT sektor endeksi" (daha hassas,
# goreceli guc hesabi icin onerilen) sutunu KULLANILDI. Iki en yeni/
# beklenmedik iddia (ASELS->XUTEK, XPTIC'in varligi) AYRICA arastirilip
# DOGRULANDI (iki bagimsiz kaynak). TAVHL/SISE bu surumde XHOLD'dan
# XULAS/XTAST'a TASINDI - kullanicinin FAALIYET BAZLI gerekcesi (TAVHL
# hava limani ISLETMECILIGI yapiyor, sirket adinda "Holding" gecmesine
# RAGMEN) BIST endeksleri CAKISABILDIGI icin ikisi de "doğru" olabilir,
# ama goreceli GUC olcumu icin faaliyet-bazli daha ANLAMLI - kullanicinin
# tercihi BENIMSENDI.
# NOT: Bazi gruplar TEK UYELI (XMADN, XUTEK, XTAST, XILTM, XINSA, XELKT,
# XGMYO) - bu semboller icin "sektor gucu" fiilen KENDI goreli guclerine
# esit olur (cesitlendirme faydasi yok), ama HERKESE kapsam saglamak
# icin boyle birakildi - hic kapsamamaktan iyi.
SEKTORLER = {
    "XMESY": ["ASTOR", "TOASO", "FROTO", "OTKAR"],
    "XHOLD": ["ALARK", "KCHOL", "SAHOL"],
    "XMADN": ["TRMET"],
    "XGIDA": ["AEFES", "ULKER"],
    "XBANK": ["HALKB", "AKBNK", "YKBNK", "VAKBN", "GARAN", "ISCTR"],
    "XUTEK": ["ASELS"],
    "XPTIC": ["BIMAS", "MGROS"],
    "XULAS": ["PGSUS", "THYAO", "TAVHL"],
    "XTAST": ["SISE"],
    "XKMYA": ["TUPRS", "PETKM"],
    "XILTM": ["TTKOM"],
    "XINSA": ["ENKAI"],
    "XELKT": ["ENJSA"],
    "XMANA": ["EREGL", "KRDMD"],  # KRDMD SEMBOLLER disinda, yalniz sektor-gucu yardimcisi
    "XGMYO": ["EKGYO"],
}
...
def main():
    ...
    rapor = {
        "calisma": "Bilesik Uyum Skoru v5 - KGS Kalite Kontrollu",
        ...
        "v4den_degisiklik": (
            "TAM KAPSAM (kullanicinin kendi arastirdigi, 30 hissenin "
            "tamaminin resmi ALT sektor endeksiyle eslestigi tablo "
            "uygulandi). Iki en yeni/beklenmedik iddia AYRICA arastirilip "
            "DOGRULANDI: ASELS->XUTEK (iki bagimsiz kaynak, XUTEK "
            "bilesen listesinde ASELS acikca var) ve XPTIC'in VARLIGI "
            "(BIST Perakende Ticaret, 2026'da olusturulmus, gercek "
            "izlenen bir endeks). TAVHL/SISE bu surumde XHOLD'dan "
            "XULAS/XTAST'a TASINDI - kullanicinin FAALIYET BAZLI "
            "gerekcesi (orn. TAVHL hava limani isletmeciligi yapiyor, "
            "adinda 'Holding' gecmesine RAGMEN) goreceli guc olcumu "
            "icin daha ANLAMLI kabul edildi. TCELL ve AKSEN bu listede "
            "YOK - kullanicinin 30 sembollük tam listesinde bulunmadiklari "
            "icin cikarildi, onceki surumlerde YANLISLIKLA eklenmislerdi."
        ),
        "kapsam_durumu": (
            "ARTIK TAM KAPSAM - SEMBOLLER'deki 30 hissenin TAMAMI 15 "
            "resmi BIST alt-sektor endeksine atandi. Bazi gruplar TEK "
            "UYELI (XMADN/TRMET, XUTEK/ASELS, XTAST/SISE, XILTM/TTKOM, "
            "XINSA/ENKAI, XELKT/ENJSA, XGMYO/EKGYO) - bu semboller icin "
            "sektor gucu fiilen kendi bireysel goreli guclerine esittir "
            "(cesitlendirme faydasi yok), ama en azindan skorlaniyorlar."
        ),
        "durustluk_notu": (
    ...
    with open("data/backtest/bilesik_uyum_skoru_v5_sonuc.json", "w", encoding="utf-8") as f:
    ...
    print("\nTamamlandi -> data/backtest/bilesik_uyum_skoru_v5_sonuc.json")
